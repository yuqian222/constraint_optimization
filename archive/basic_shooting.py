...
def main():
    args = get_args()
    dir_name = "results/%s/%s-%s"%(args.env, "basic_model", strftime("%m_%d_%H_%M", gmtime()))
    ...
    dynamics = DynamicsEnsemble(args.env, num_models=1)
    ...
    for i_episode in count(1):
        ...
        if args.correct and i_episode>1:
            ...
            for s, a, r in state_action_rew:
                max_a = dynamics.shoot(s, sample_policy)
        ...
        for branch in range(args.branches):
            ...
            eval_rew /= EVAL_TRAJ
            dyna_eval = dynamics.evaluate(branch_policy)
